...
function App() {
    ...
    useEffect(() => {
        const getRawSchedule = () => {
            ...
            // SANITIZE DATA (Preserves embedUrl)
            const sanitize = (list) => (!list ? [] : list.map(m => ({
                ...
                // CRITICAL: Ensure embedUrl is kept
            ...
            <main className="relative z-10 pt-16">
                {searchQuery ? (
                    ...
                    <>
                        {activePage === 'home' && (
                            <>
                                ...
                                <div className="z-20 relative mt-8 pb-20">
                                    {/* USE MOVIE ROW FOR HOME PAGE */}
                                    <MovieRow genre={{ name: "Recently Added", id: 'custom' }} movies={safeRecent} onMovieClick={setSelectedMovie} />
                                    {displayGenres.map(genre => (
                                        <div key={genre.id} id={`genre-${genre.id}`}>
                                            <MovieRow genre={genre} movies={safeVault} onMovieClick={setSelectedMovie} />
                                        </div>
                                    ))}
                                </div>
                            </>
                        ...
                        {activePage === 'movies' && <div className="pt-8 px-6"><MovieGrid title="The Vault" movies={safeVault} onMovieClick={setSelectedMovie} /></div>}
                        {activePage === 'recent' && <div className="pt-8 px-6"><MovieGrid title="Recently Added" movies={safeRecent} onMovieClick={setSelectedMovie} /></div>}
                        {activePage === 'popular' && <div className="pt-8 px-6"><MovieGrid title="Coming Soon" movies={safeComingSoon} onMovieClick={() => {}} /></div>}
                           ...
                           <div className="pt-24 px-6 md:px-20 max-w-4xl mx-auto text-slate-300 pb-20">
                                ...
                                <div className="space-y-12 text-lg leading-relaxed">
                                    ...
                                    <section>
                                        ...
                                        <p>Selected titles on the platform have undergone AI-assisted restoration, improving image stability, clarity, and audio balance while maintaining the integrity of the original material. No narrative, visual, or editorial modifications are introduced.</p>
                                    </section>
                                    <section>
                                        <h3 className="text-2xl font-bold text-white mb-4 border-l-4 border-red-600 pl-4">Film Sources and Restoration</h3>
                                        <p className="mb-4">All films hosted on Anreal Cinema are verified to be:</p>
                                        <ul className="list-disc pl-6 space-y-2 mb-4 text-slate-400">
                                            <li>In the public domain, or</li>
                                            <li>Free of active copyright restrictions</li>
                                        </ul>
                                        <p>Where available, original or historically accurate cuts are used. In some cases, enhanced versions are presented to improve viewing quality on modern displays.</p>
                                    </section>
                                    <section>
                                        <h3 className="text-2xl font-bold text-white mb-4 border-l-4 border-red-600 pl-4">Purpose</h3>
                                        <p className="mb-4">Anreal Cinema is not a commercial streaming service and does not host newly released or licensed studio content. It exists as a preservation-focused archive intended for:</p>
                                        <ul className="list-disc pl-6 space-y-2 mb-4 text-slate-400">
                                            <li>Education</li>
                                            <li>Research</li>
                                        ...
                                        <p>The platform is designed to operate quietly in the background, allowing the films themselves to remain the focus.</p>
                                    </section>
