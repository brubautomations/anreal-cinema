...
export default function MovieCard({ movie, onClick }) {
    ...
    return (
        <div
            ...
            className={`relative flex-none w-44 md:w-72 aspect-[2/3] group cursor-pointer overflow-hidden rounded-md transition-all duration-500 hover:z-10 shadow-lg bg-slate-900 select-none ${isComingSoon ? 'grayscale-[0.8] cursor-not-allowed opacity-60 hover:scale-100' : 'hover:scale-105'
                }`}
        >
            {!imageError ? (
                <img
            ...
            {/* Price/Download info (optional, removed for cleaner look, but user might want it) */}
            {!isComingSoon && (
                <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-sm px-2 py-1 rounded text-[10px] text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity">
                    {Math.round(movie.downloads / 1000)}k DLs
                </div>
            )}

            {/* Corner Tag for Coming Soon */}
            {isComingSoon && (
